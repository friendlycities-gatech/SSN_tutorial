...
`tmap` allows users to view their maps in interactive modes with pre-defined Leaflet basemap styles, such as OpenStreetMap, Esri.WorldGrayCanvas, and Esri.WorldTopoMap. You can change basemap styles through widgets in the RStudio Plots panel, or defined through `tm_basemap()` argument. This is a quick and easy way to generate preliminary maps with base maps for presentations and illustrations. The interactive mode can also help you examine your data in a spatial context by adding pop up labels or hover labels.
...
tmap_mode('view')
tm_shape(MafiaSpatial) + 
  tm_symbols(col='lightblue', scale=3) + 
...
token = 'YOUR MAPBOX TOKEN' #see how to apply for token in Mapbox Docs.
...
```{r echo=FALSE, fig.cap="", out.width = '100%', fig.show = "hold", class.source="Code"}
knitr::include_graphics("Figs/05-aesthetics-1.png")
```
You can find all the available Mapbox style [here](https://docs.mapbox.com/api/maps/styles/). You can also customize your Mapbox basemap in [Mapbox Studio](https://studio.mapbox.com/). To use the customized base map, replace the `map_type` in the `set_defaults` function with your customized style URL ID (can be found after the path https://api.mapbox.com/styles/), which you can find if you click "share" button for your customized style. The following figures shows the effects of `map` object under `map_type = 'light'` (top left), `map_type = 'dark'` (top right), `map_type = 'outdoors'` (bottom left), `map_type = 'satellite'` (bottom right).   

```{r echo=FALSE, fig.cap="", out.width = '50%', fig.show = "hold", class.source="Code"}
knitr::include_graphics("Figs/05-aesthetics-1.png")
