...
## Flattening Ratio
**Global Flattening Ratio** is a network-level metric to measure the spatial tightness of a network. We reference the following definition in the paper [Metrics for characterizing network structure and node importance in Spatial Social Network (Sarkar et al., 2019)](https://www.tandfonline.com/doi/full/10.1080/13658816.2019.1567736). To define the flattening ratio, first create a degree-constrained nearest neighbour network \bar{G} from the given social network G by reconfiguration, such that each node *i* in \bar{G} with degree *K* connects to its nearest *K* neighbors in Euclidean space. As such, the (global) flattening ratio is the ratio of the sum of the Euclidean distance of edges in \bar{G} where all nodes are connected to their K-nearest neighbors and the sum of the Euclidean distance of actual edges in G. 
